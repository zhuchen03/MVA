...
def main(args, init_distributed=False):
    ...
    experiment = setup_comet(args, init_distributed)
    ...
    while (
        ...
        # train for one epoch
        valid_losses = train(args, trainer, task, epoch_itr, max_update, experiment)
...
def train(args, trainer, task, epoch_itr, max_update=math.inf, experiment=None):
        ...
        if num_updates % args.log_interval == 0:
            ...
            if experiment:
                for key, val in stats.items():
                    if isinstance(val, meters.AverageMeter):
                        val = val.avg
                    if val is not None:
                        experiment.log_metric(key, val, step=num_updates)

            # reset mid-epoch stats after each log interval
            # the end-of-epoch stats will still be preserved
            metrics.reset_meters('train_inner')

        valid_losses = validate_and_save(args, trainer, task, epoch_itr, valid_subsets, experiment=experiment)
...
def validate_and_save(args, trainer, task, epoch_itr, valid_subsets, experiment=None):
    num_updates = trainer.get_num_updates()
    do_save = (
        (
    ...
    valid_losses = [None]
    if do_validate:
        valid_losses = validate(args, trainer, task, epoch_itr, valid_subsets, experiment=experiment)
    # Save
    if do_save:
...
def validate(args, trainer, task, epoch_itr, subsets, experiment=None):
    ...
    for subset in subsets:
        ...
        if experiment:
            # metrics = {'val_{}'.format(k): v for (k, v) in stats.items()}
            # experiment.log_metrics(metrics, step=trainer.get_num_updates())
            for key, val in stats.items():
                if isinstance(val, meters.AverageMeter):
                    val = val.avg
                if val is not None:
                    experiment.log_metric("val_{}".format(key), val, step=trainer.get_num_updates())
