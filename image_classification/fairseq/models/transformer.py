...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        # fmt: on
        parser.add_argument('--use-bn', default=False, action="store_true")
        parser.add_argument('--bn-betas', default=(0.9, 0.9), type=eval)
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if args.encoder_normalize_before:
            if args.use_bn:
                self.layer_norm = RectifyBatchNorm(embed_dim, betas=args.bn_betas)
            else:
        ...
        if getattr(args, "layernorm_embedding", False):
            if args.use_bn:
                self.layernorm_embedding = RectifyBatchNorm(embed_dim, betas=args.bn_betas)
            else:
    ...
    def forward_embedding(self, src_tokens):
        ...
        if self.layernorm_embedding is not None:
            if self.args.use_bn:
                encoder_padding_mask = src_tokens.neq(self.padding_idx)
                x = self.layernorm_embedding(x, mask=encoder_padding_mask.transpose(0,1).contiguous().unsqueeze(2))
            else:
    ...
    def forward(
        ...
        if self.layer_norm is not None:
            if self.args.use_bn:
                x = self.layer_norm(x, mask=~encoder_padding_mask.transpose(0,1).contiguous().unsqueeze(2)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features(
        ...
        if self.layernorm_embedding is not None:
            if self.args.use_bn:
                mask = prev_output_tokens.neq(self.padding_idx)
                x = self.layernorm_embedding(x, mask=mask.transpose(0, 1).contiguous().unsqueeze(2))
            else:
                x = self.layernorm_embedding(x)
        ...
        if self.layer_norm is not None:
            if self.args.use_bn:
                x = self.layer_norm(x, mask=~self_attn_padding_mask.transpose(0, 1).contiguous().unsqueeze(2)
...
@register_model_architecture("transformer", 'transformer_prenorm_deep_wmt_en_de')
